  ...
  return mockRequest.post('/api/v1/products')
    ...
    .then(data=>{
      return mockRequest.put(`/api/v1/products/${data.body._id}`)
        .send({ categoryName:'food',name: 'Item  UPDATED', price: 11, quantityInStock: 666})
        .then(results=>{
          expect(results.status).toBe(200);
          expect(results.body.name).toEqual('Item  UPDATED');
          expect(results.body.price).toEqual(11);
          expect(results.body.quantityInStock).toEqual(666);
        });
    });
});